...
namespace TiaMcpServer.Siemens
{
    ...
    ///
    /// API 形态是上游**反射实测**出来的（2026-09-03，V21），不是推理的：
    ...
    ///
    /// 本仓适配点：上游用 GetDeviceItemByPath，本仓是 GetDeviceItem（同一职责）。
    /// </summary>
    public partial class Portal
    {
        ...
        ///
        /// ⚠ 必须整体跑在 _sta.Run 里：item.Addresses 取出的是 Openness 的 RCW，
        /// 一旦跨线程读它的属性就是 "Cross-thread operation is not valid in Openness within STA"。
        /// 本方法在 STA 线程内就把它提取成普通 POCO（IoAddressInfo）再返回，RCW 不外泄。
        /// （上游那份没包，本仓实测必炸 —— 这是真机验证才暴露出来的。）
        /// </summary>
        public IReadOnlyList<IoAddressInfo>? GetDeviceItemAddresses(string deviceItemPath)
        {
            return _sta.Run<IReadOnlyList<IoAddressInfo>?>(() =>
            {
        ...
        /// 于是去查一个没有问题的组态。
        /// 返回 "子项名 → 地址摘要" 的清单，只下钻一层（再深就变成整棵树，噪音大于信息）。
        /// </summary>
        ...
        {
            // 同样必须整体在 STA 线程上：Address 是 RCW，写它的 StartAddress 跨线程必炸。
            return _sta.Run<(bool ok, string message, IoAddressInfo? before, IoAddressInfo? after)>(() =>
            {
