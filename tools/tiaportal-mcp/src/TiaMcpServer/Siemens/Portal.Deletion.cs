...
namespace TiaMcpServer.Siemens
{
    ...
    ///
    /// 三条共同规矩：
    /// 1) 路径必须精确 —— 叶子名含正则元字符一律拒绝。定位可能是「先字面、再锚定正则」，
    ///    模式仍可能命中到一个**同样合法但不是你要的**对象；读错了顶多返回错东西，删错了工程就没了。
    ///    所以删除口不吃模式，一个字符都不行。
    ...
    /// PLC 侧编译不一定报错，故障要到画面上才暴露。
    ///
    /// ⚠ 本仓适配：所有对外方法整体跑在 _sta.Run 里（RCW 不外泄）。
    /// 上游那份没包，本仓会直接报 "Cross-thread operation is not valid in Openness within STA"
    /// （已在地址族上实测到）。
    /// </summary>
    public partial class Portal
    {
        ...
        public JsonObject DeletePlcBlock(string softwarePath, string blockPath, bool dryRun)
        {
            ...
            var block = GetBlockRcw(softwarePath, blockPath);
            if (block == null)
            {
                // 打错名字和「块确实不存在」对调用方是两件事，把可选项列出来才好改。
                // 本仓 GetBlocks 返回 DTO（ResponseBlockInfo）而不是 RCW，取它的 Path 就是给调用方看的路径。
                var known = GetBlocks(softwarePath);
                var names = known?.Select(b => b?.Path ?? b?.Name ?? "")
            ...
            bool absent = GetBlockRcw(softwarePath, resolvedPath) == null;
            result["deleted"] = true;
        ...
        /// <summary>
        /// 预览或删除一张 PLC 变量表。tagTableName 接受裸表名或 GetPlcTagTables 返回的组限定名，
        /// 反斜杠也当分隔符。dryRun=true（默认）只解析并清点内容，不做任何改动。
        /// </summary>
        ...
        public JsonObject DeletePlcType(string softwarePath, string typePath, bool dryRun)
        {
            ...
            PlcType? type = GetTypeRcw(softwarePath, typePath);
            if (type == null)
            {
                throw new PortalException(PortalErrorCode.NotFound,
        ...
        // ── 本仓补充的 helper（上游有、本仓没有的两个）──────────────────────

        /// <summary>把交叉引用条目排成 JsonArray（上游在别处定义，本仓就放这里，只有删除族用得着）。</summary>
        ...
        /// <summary>
        /// 取变量表遍历的根对象。本仓既有写法（见 Portal.CausalTrace.cs）：
        /// 软件层可能直接给 TagTables，也可能先给 TagTableGroup/TagTableFolder —— 两者都兜。
        /// </summary>
        private static object? ResolvePlcTagTableGroup(object plc)
        {
            if (plc == null) return null;
            return TryGetPropertyValue(plc, "TagTableGroup", "TagTableFolder") ?? plc;
        }

        /// <summary>
        /// 和 Portal.Software.cs 里的遍历走同一棵树、同一套匹配规则，
        ...
        /// 已知事实（真机实测）：Software / Device / DeviceItem 三层都回 "service not available"，
            ...
            object table, string resolvedPath, out string? reason)
        {
            ...
            if (svc == null)
            {
                reason = "PlcTagTable 给不出 CrossReferenceService（已知 Software/Device/DeviceItem "
