...
namespace TiaMcpServer.Siemens
{
    ...
    ///
    /// 为什么单独一份：整机添加走 Devices.CreateWithItem，插子模块走的是完全不同的一套
    /// （HardwareObject.PlugNew），失败模式也不一样 —— 整机失败是「订货号不存在」，
    /// 插子模块失败还多出「槽位被占」和「这台 CPU 不接受这块板」两种，必须分开报。
    ///
    /// API 形态是上游**反射实测**出来的（V21，Siemens.Engineering.Base.dll），本仓 V20 实测同样具备：
    ...
    /// 注意 DeviceItemComposition 上**没有**任何 Create/PlugNew 方法，所以插模块只能走宿主 HardwareObject。
    ///
    /// **槽位号不写死**：S7-1200 信号板的槽位号由 GetPlugLocations() 在运行时报出来，
    /// 引擎不猜也不硬编码任何 CPU 的槽位表。
    ///
    /// ⚠ 本仓适配：所有对外方法整体跑在 _sta.Run 里（RCW 不外泄）。上游那份没包，
    /// 本仓会直接报 "Cross-thread operation is not valid in Openness within STA"（已在地址族上实测到）。
    /// </summary>
    public partial class Portal
    {
        ...
        /// <summary>插入子模块的结果。失败时 Reason 给出**可判定的失败类别**，不是一句「插入失败」。</summary>
        ...
        {
            return _sta.Run<(IReadOnlyList<PlugLocationInfo> free, IReadOnlyList<PluggedItemInfo> occupied)?>(() =>
            {
                ...
                var item = GetDeviceItem(deviceItemPath);
                if (item == null)
                {
                    return null;
                }
        ...
        /// <param name="orderNumber">订货号，例如 6ES7221-3BD30-0XB0，带不带空格都行。也可直接传完整 TypeIdentifier。</param>
            ...
            string deviceItemPath, string orderNumber, string version, int positionNumber, string? name, bool dryRun)
        {
            ...
            var host = GetDeviceItem(deviceItemPath);
            if (host == null)
            {
            ...
            foreach (var slot in slotCandidates)
            {
                foreach (var typeId in typeIdentifiers)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        var again = GetDeviceItem(deviceItemPath);
                        if (again == null)
                        {
            ...
            var verifyHost = GetDeviceItem(deviceItemPath);
            if (verifyHost == null)
            {
