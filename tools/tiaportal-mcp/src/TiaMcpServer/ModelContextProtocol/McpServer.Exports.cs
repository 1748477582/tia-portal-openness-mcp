...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    //
    //  ⚠ 与上游的差异：**本线没有 CallTool / FindTools 这两个元工具**（属补齐清单
    //  B 类的"元工具"项，尚未做）。所以这里**没有**上游那套「CallTool 转发目标也放行」
    //  的判断 —— 等元工具补上时必须一并加回来，否则 CallTool 转发到分页工具时，
    //  那一页会被再寄存一次，变成"句柄的句柄"，越翻越远。
    // ───────────────────────────────────────────────────────────────────────────
    public static partial class McpServer
    {
        ...
        /// <summary>给每个工具包上大响应寄存层。签名固定：Program.cs 按这个形状调。</summary>
            ...
            [Description("length: how many characters to return. 0 or omitted = this session's response limit.")] int length = 0)
        {
            ...
            if (slice.Error != null)
            {
                ...
                // 取不到就抛，让宿主标 IsError，别返回一份空 Message 让模型误以为
                // 「这份导出是空的」。
                throw new McpException(
                    slice.Message ?? $"取不到句柄 {exportId}（{slice.Error}）。",
                    null, McpErrorCode.InvalidParams);
            }
            ...
            [Description("overwrite: DEFAULT false — if the file already exists the call is REFUSED rather than replacing it. Pass true only after the user agreed to overwrite that specific file.")] bool overwrite = false)
        {
            ...
            {
                throw new McpException("outputPath 不能为空。", null, McpErrorCode.InvalidParams);
            }
    ...
    internal sealed class ResponseGuardTool : McpServerTool
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                return Shrink(result, name, target);
            }
            catch
            {
                // 瘦身失败绝不能吃掉本来正常的结果。
        ...
        /// 就地放一份 —— 只有这里用得着，挂到 McpServer 上反而会跟别的回流文件撞名。</summary>
